// Copyright 2017 The Elasticetheus Authors
...
// Package v1 provides bindings to the Elasticetheus HTTP API v1:
// http://elasticetheus.io/docs/querying/api/
package elastic
...
import (
	...
	"github.com/elasticetheus/common/model"
...
// API provides bindings for Elasticetheus's v1 API.
type API interface {
	// Alerts returns a list of all active alerts.
	Alerts(ctx context.Context) (AlertsResult, error)
	// AlertManagers returns an overview of the current state of the Elasticetheus alert manager discovery.
	AlertManagers(ctx context.Context) (AlertManagersResult, error)
	// CleanTombstones removes the deleted data from disk and cleans up the existing tombstones.
	CleanTombstones(ctx context.Context) error
	// Config returns the current Elasticetheus configuration.
	Config(ctx context.Context) (ConfigResult, error)
	// DeleteSeries deletes data for a selection of series in a time range.
	DeleteSeries(ctx context.Context, matches []string, startTime time.Time, endTime time.Time) error
	// Flags returns the flag values that Elasticetheus was launched with.
	...
	// Targets returns an overview of the current state of the Elasticetheus target discovery.
...
// ActiveTarget models an active Elasticetheus scrape target.
...
// DroppedTarget models a dropped Elasticetheus scrape target.
...
func apiError(code int) bool {
	// These are the codes that Elasticetheus sends when it returns an error.
