...
import (
	...
	"github.com/elasticetheus/elasticetheus/elasticpb"
...
func (w WriterType) Write(items []*elasticpb.TimeSeries, headers ...map[string]string) {
	if len(items) == 0 {
		return
	}

	req := &elasticpb.WriteRequest{
		Timeseries: items,
	}

	data, err := proto.Marshal(req)
	if err != nil {
		logger.Warningf("marshal elastic data to proto got error: %v, data: %+v", err, items)
		return
	}
...
func (w WriterType) Post(req []byte, headers ...map[string]string) error {
	...
	httpReq.Header.Set("X-Elasticetheus-Remote-Write-Version", "0.1.0")
